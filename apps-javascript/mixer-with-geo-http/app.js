    ...
    this.handle_request = function(request, response) {
        ...
        if (((settings.use_sonar_availability_threshold === true && Object.keys(dataFusion).length > 0) || settings.use_sonar_availability_threshold === false)
            ...
            if (candidateAliases.length === 0) {
                ...
                candidateAliases = Object.keys(candidates);
                if (candidateAliases.length > 0) {
                    body = getHighestProvidersOrdered(candidates, 'avail');
                }
            } else if (candidateAliases.length === 1) {
                body.push(candidateAliases[0]);
                ...
                if (candidateAliases.length > 0) {
                    ...
                    body = getHighestProvidersOrdered(candidates, 'score');
        ...
        if (body.length === 0) {
            ...
            candidateAliases = Object.keys(candidates);
            body = getRandomProviders(candidateAliases);
        ...
        for (var i= 0; i < body.length; i++) {
            ...
            cname = selectedCandidates
                && selectedCandidates[body[i]]
                && selectedCandidates[body[i]].cname
                ? selectedCandidates[body[i]].cname
                : settings.providers[body[i]].cname;

            response.addProviderHost(body[i], cname);
        }
