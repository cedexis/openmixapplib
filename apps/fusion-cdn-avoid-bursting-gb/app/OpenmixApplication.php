...
class OpenmixApplication implements Lifecycle {
    ...
    *
    * 'threshold'
    * Monthyly GB usage commit threshold. If current usage exceeds this commit, apply
    * padding to the CDN that is receiving too much traffic.
    *
    * 'multiplier'
    ...
    * to the percent over the threshold when calculating the padding?
    ...
    */
    public $burstable_cdns = array(
        'akamai' => array('threshold' => 20000, 'multiplier' => 1.5)
    );
    ...
    public function init($config) {
        $config->declareInput(FusionProperties::GB, implode(',', array_keys($this->burstable_cdns)));
    ...
    public function service($request, $response, $utilities) {
        ...
        if (is_array($gb)) {
            foreach ($this->burstable_cdns as $cdn => $settings) {
                if (array_key_exists($cdn, $gb)) {
                    if ($gb[$cdn] > $settings['threshold']) {
                        $default_pad = $this->providers[$cdn]['padding'];
                        //print("\ndefault padding $cdn: $default_pad");
                        $padding = (floatval($gb[$cdn]) / $settings['threshold'] - 1 ) * $settings['multiplier'] * 100;
                        //print("\npadding $cdn: $padding");
                        // In case the difference is so small we end up with a fraction
                        // of a percent. Round up to 1%
